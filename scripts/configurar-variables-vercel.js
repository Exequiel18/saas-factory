...
async function main() {
  ...
  // Variables a configurar (usando 'plain' en lugar de 'system' para las que fallaron)
  const envVars = [
    {
      key: 'NEXTAUTH_URL',
  ...
  console.log(`Configurando variables en: ${PROJECT_NAME}\n`)
  ...
  for (const envVar of envVars) {
    ...
    if (setEnv.status === 200 || setEnv.status === 201) {
      ...
      console.log(`❌ ${envVar.key}: ${setEnv.text.substring(0, 100)}`)
    }
  }
